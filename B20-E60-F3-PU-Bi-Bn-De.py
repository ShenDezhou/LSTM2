...
def getUnigramVector(sentence, i):
    ngrams = getUnigram(sentence, i)
    ngramv = []
    for ngram in ngrams:
        for ch in ngram:
...
def getFeatures(sentence, i):
    features = []
    features.extend(getUnigramVector(sentence, i))
...
if MODE == 1:
    with codecs.open('plain/msr_training.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('plain/msr_train_states.txt', 'r', encoding='utf8') as fs:
            with codecs.open('model/f3/msr_train_crffeatures.pkl', 'wb') as fx:
                with codecs.open('model/f3/msr_train_crfstates.pkl', 'wb') as fy:
                    xlines = ft.readlines()
                    ylines = fs.readlines()
                    X = []
                    y = []
...
if MODE==2:
    ...
    with codecs.open('model/f3/msr_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/f3/msr_train_crfstates.pkl', 'rb') as fy:
